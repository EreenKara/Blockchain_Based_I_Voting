...
const createElection = asyncHandler(async (req, res) => {
  ...
  if (!name || !startDate || !endDate) {
    return res
      .status(400)
      .json({ message: "Başlık, başlangıç ve bitiş tarihleri zorunludur." });
  }
  ...
  try {
    ...
    if (election.accessType === "private") {
      ...
      if (userId) {
        try {
          ...
          if (
            ...
            !responseUser.data.success !== true
          ) {
            successUser = false;
            console.error(
              "Failed to sync election user access:",
      ...
      if (groupId) {
        try {
          ...
          if (
            ...
            !responseGroup.data.success !== true
          ) {
            successGroup = false;
            console.error(
              "Failed to sync election group access:",
              responseGroup.data.message
            );
          }
        } catch (error) {
          successGroup = false;
      ...
      if (successUser && successGroup) {
        ...
        return res
          .status(500)
          .json({ message: "Failed to add access for users or groups." });
      }
    } else {
      console.log(
        "Public election access configuration is not implemented yet."
      );
      return res
        .status(400)
        .json({ message: "Public election access not implemented yet." });
    }
  } catch (error) {
    console.error("Error setting election access:", error.message);
    res
      .status(500)
      .json({ message: "Error setting election access", error: error.message });
  }
};
...
const updateElectionAccess = async (req, res) => {
  ...
  const { electionId, userId, groupId } = req.body;
  console.log(
    `📢 Gelen İstek - electionId: ${electionId}, userId: ${userId}, groupId: ${groupId}`
  );
  ...
  try {
    ...
    if (election.createdBy !== user.email) {
      console.error(
        "❌ Yetkisiz erişim: Seçimi sadece oluşturan kişi güncelleyebilir."
      );
      return res.status(403).json({
        message: "Only the creator of the election can update access.",
      });
    }
    ...
    if (userId) {
      try {
        console.log(
          `🔍 Kullanıcı erişim kontrolü başlatıldı - electionId: ${electionId}`
        );
        ...
        const existingUsers = checkUserResponse.data.data.map(
          (user) => user.userId
        );
        if (!existingUsers.includes(userId)) {
          ...
          if (
            responseUser.status !== 200 ||
            !responseUser.data ||
            responseUser.data.message.includes("başarıyla")
          ) {
            console.log(
            ...
            successUser = false;
            console.error(
              "❌ User erişim ekleme başarısız:",
              responseUser.data.message
            );
          }
        } else {
          console.log(
            "ℹ Kullanıcı zaten erişime sahip, işlem atlanıyor:",
            userId
          );
        }
      } catch (error) {
    ...
    if (groupId) {
      try {
        console.log(
          `🔍 Grup erişim kontrolü başlatıldı - electionId: ${electionId}`
        );
        ...
        console.log(
          "✅ Grup erişim kontrolü API cevabı:",
          checkGroupResponse.data
        );

        const existingGroups = checkGroupResponse.data.data.map(
          (group) => group.groupId
        );
        if (!existingGroups.includes(groupId)) {
          ...
          if (
            responseGroup.status !== 200 ||
            !responseGroup.data ||
            responseGroup.data.message.includes("başarıyla")
          ) {
            console.log(
    ...
    if (successUser && successGroup) {
      ...
      return res
        .status(200)
        .json({ message: "Election access updated successfully", election });
    } else {
      console.error("❌ Kullanıcı veya grup erişimi güncellenemedi.");
      return res
        .status(500)
        .json({ message: "Failed to update access for users or groups." });
    }
  } catch (error) {
    console.error("❌ Genel hata:", error.message);
    res.status(500).json({
      message: "Error updating election access",
      error: error.message,
    });
  }
};
...
const addChoiceToElection = asyncHandler(async (req, res) => {
  ...
  if (!choiceIds || !Array.isArray(choiceIds) || choiceIds.length === 0) {
    return res.status(400).json({
      message: "choiceIds array olarak gönderilmeli ve boş olmamalı.",
    });
  }
...
const updateElectionStatus = asyncHandler(async (req, res) => {
  ...
  if (election.status === "active") {
    ...
    return res.status(200).json({
      message: "Election status updated to COMPLETED successfully",
      election,
    });
  }
  ...
  return res.status(400).json({
    message:
      "Election status update is invalid. Only ACTIVE elections can be completed.",
  });
});
...
const getActiveElection = asyncHandler(async (req, res) => {
  ...
  if (now >= election.startDate && now <= election.endDate) {
    ...
    return res.status(400).json({
      message: "Election cannot be started outside of start and end dates",
    });
  }
});
