const {createNeighbourhood,getNeighbourhoodList,getNeighbourhoodById}=require("../services/neighbourhoodService");

const createNeighbourhoodController=async(req,res)=>{
try{
    await createNeighbourhood(req,res);
}catch(error){
    res.status(500).json({message:"an error occured while creating the neighbourhood"})
}
};
...
module.exports={createNeighbourhoodController,neighboorhoodListAll,getNeighbourhoodByIdController};