const {addAccessUserToElection}=require("../services/electionAccessUsersService");
...
const getUsersWithAccessToElectionController = async (req, res) => {
    try {
        ...
        console.error("Hata:", error.message);
        res.status(500).json({ message: "Seçime erişimi olan kullanıcıları getirirken hata oluştu.", error: error.message });
    }
};
