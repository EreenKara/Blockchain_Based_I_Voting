...
const addAccessUserToElection = async (electionId, userId = null, groupId = null, token) => {
    try {
        ...
        if (!user || !user.email) {
            return { success: false, message: "Erişim reddedildi: Geçersiz kullanıcı." };
        }
        ...
        if (groupId) {
            ...
            const existingGroupAccess = await ElectionAccessUsers.findOne({
            ...
            // Grubu seçime ekle
            await ElectionAccessUsers.create({ electionId, groupId });
...
const getUsersWithAccessToElection = async (electionId) => {
  try {
      ...
      if (!users.length) {
          return { success: false, message: "Bu seçim için erişime sahip kullanıcı bulunamadı." };
      }
